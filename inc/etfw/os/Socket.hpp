...
#ifndef OS_SOCK_MAX_HOST_LEN
#define OS_SOCK_MAX_HOST_LEN    40
#endif
...
#include "OsTypes.hpp"
    ...
    class Sock
    {
        public:
            ...
            struct Address
            {
                static constexpr size_t MaxHostStrLen = OS_SOCK_MAX_HOST_LEN;
                using HostStr_t = etl::string<MaxHostStrLen>;
                ...
                Address(const char *addr, SockPort_t port):
                    Addr(addr),
                    Port(port)
                {}

                template <size_t LEN>
                Address(etl::string<LEN>& addr, SockPort_t port):
                    Addr(addr),
                    Port(port)
                {}

                inline void update(const char *addr, SockPort_t port)
                {
                    Addr = addr;
                    Port = port;
                }
                ...
                template <size_t LEN>
                inline void update(etl::string<LEN>& addr, SockPort_t port)
                {
                    Addr = addr;
                    Port = port;
                }
